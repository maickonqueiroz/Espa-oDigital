...
±±ºUso       ³ Itup / Gefco                                               º±±
±±-----------³------------------------------------------------------------º±±
±±FTL        ³ Novo formato de calculo do FTL - Frank Z Fuga - 02/05/16   º±±
±±-----------³------------------------------------------------------------º±±
±±FTL        ³ Mudancao do calculo FTL tipo 6 - Frank Z Fuga - 04/05/16   º±±
...
//			_lPorKMPeso := .F. // Frank em 02/03/16
		...
		If _aRet[1] == "N" .and. _aRet[3] == "S" // Tratamento especial para rateio line haul e FTL - Frank 05/04/16
			...
			If _cTipo == "FTL"
                ...
                // Atualiza o calculo da ZA6_EKMVEN
				U_AchaKM()               
				U_FIATFTL()
				_nTotKm := oDlgTra:aCols[oDlgTra:nAt][Ascan(oDlgTra:aHeader,{|x|AllTrim(x[2])=="ZA6_EKMVEN"})] 

				/*                                     
				Premissa: 
				1) a tabela ZTC sera posicionada com base no ZA6_EKMVEN
				2) o total de KM vem do campo ZA6_EKMVEN
				
            	CASO 1
            	Uma unica linha com PECA                    
            		se ZTC_VRFIXO > 0
            			ZA7_VALOR := ZTC_VRFIXO
            		se nao
            			ZA7_VALOR := ZTC_VRKMON * TOTKM 
            		fim-se
            	
            	CASO 2
            	Uma unica linha com RETORNO
            		se ZTC_VRFIXR > 0
            			ZA7_VALOR := ZTC_VRFIXR
            		se ZTC_PERR > 0
            			ZA7_VALOR := ZTC_VRFIXO + (ZTC_VRFIXO * ZTC_PERR)/100
            		se ZTC_VRKMRO > 0
            			ZA7_VALOR := ZTC_VRKMRO * TOTKM
            		se ZTC_PEKMRO > 0
            			ZA7_VALOR := ( ZTC_VRKMON + ( ZTC_VRKMON * ZTC_PEKMO)/100 ) * TOTKM
            		fim-se
            	
            	CASO 3
            	Duas linhas uma de PECA e outra de RETORNO
					Forma antiga
            		X  := calculo do CASO 2
            		X1 := X * ZT0_PRTEMB
            		X2 := X * (100 - ZT0_PRTEMB)
            		Encontrar a peca e aplicar o valor X2 
            		Encontrar o retorno e aplicar o valor X1
            		
            		Forma nova
            		PASSO 1 - Achar o valor total
    				_nTotal := 0
	           		If _VRFIXR > 0
            			_nTotal := _VRFIXR
            		EndIF
            		If _PERR > 0
            			_nTotal := _VRFIXO + (_VRFIXO * _PERR)/100
            		EndIf
            		If _VRKMRO > 0
            			_nTotal := _VRKMRO * _nTotKm
            		EndIf
            		If _PEKMRO > 0
            			_nTotal := ( _VRKMON + ( _VRKMON * _PEKMRO)/100 ) * _nTotKm
            		EndIf    
            		
            		PASSO 2 - Achar o valor total das pecas
            		_nPecas := 0
            		If _VRFIXO > 0 
            			_nPecas := _VRFIXO
            		Else
            			_nPecas := _VRKMON * _nTotKm
            		EndIf      
            		
            		PASSO 3 - Achar o valor total do retorno
            		_nRetorno := _nTotal - _nPecas
            	
            	CASO 4
            	Duas linhas ou mais somente de PECA
            		x := calculo do CASO 1
            		y := Total de peso da ZA7
            		
            		Posicionar na primeira PECA
            		ZA7_VALOR := X * ((ZA7_PESO * 100)/y)/100
            		
            		Posicionar na segunda PECA
            		ZA7_VALOR := X * ((ZA7_PESO * 100)/y)/100
            		...
            	
            	CASO 5
            	Duas linhas de RETORNO
            		x := calculo do CASO 2
            		y := Total de peso da ZA7
            		
            		Posicionar no primeira RETORNO
            		ZA7_VALOR := X * ((ZA7_PESO * 100)/Y)/100
            		
            		Posicionar no segunda RETORNO
            		ZA7_VALOR := X * ((ZA7_PESO * 100)/Y)/100
            		...                          
            		
				CASO 6            	
            	Quatro linhas Duas de PECA e Duas de RETORNO
            		x = Executar o calculo CASO 3
            		
            		Separar todos as pecas                
            		y := Total de peso da ZA7 (somente de pecas)
            		...
            		Separar todos os retornos
            		y := Total de peso da ZA7 (somente de retornos)
                ...
                */                  
				...
				// Verificar as variacoes dos tipos de viagens na ZA7
                _nTotLin  := 0
                _nTotPeca := 0
                _nTotReto := 0
           		For _nX:=1 to Len(oDlgCar:aCols)
				...
				// Verificar a faixa da tabela ZTC
				dbSelectArea("ZTC")
				DbOrderNickName("ITUPZTC001")
				dbSeek(xFilial("ZTC")+ZT0->ZT0_CODTAB+ZT0->ZT0_VERTAB+ZT0->ZT0_ITEMTB)
				...
				_lPassou := .F.
				While !Eof() .and. ZTC_FILIAL+ZTC_TABVEN+ZTC_VERVEN+ZTC_ITTABV == xFilial("ZTC")+ZT0->ZT0_CODTAB+ZT0->ZT0_VERTAB+ZT0->ZT0_ITEMTB
					If ZTC_FAIXAD <= _nTotKm .and. ZTC_FAIXAA >= _nTotKm					
						_VRKMON		:= ZTC_VRKMON
						_VRFIXO 	:= ZTC_VRFIXO
						_VRKMRO 	:= ZTC_VRKMRO
						_PEKMRO  	:= ZTC_PEKMRO
				...
				EndDo               
				If !_lPassou
					MsgStop("Não foi encontrado a faixa na tabela de vendas","Atenção!")
				EndIF                                   

				// CASO 1
				If _nTotLin == 1 .and. _nTotPeca == 1
					_nValor := 0
            		If _VRFIXO > 0 
            			_nValor := _VRFIXO
            		Else
            			_nValor := _VRKMON * _nTotKm
            		EndIf
            		
            		// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nValor
						EndIF
					Next
				...
				If _nTotLin == 1 .and. _nTotReto == 1
            		...
            		EndIF
            		If _PERR > 0
            			_nValor := _VRFIXO + (_VRFIXO * _PERR)/100
            		EndIf
            		If _VRKMRO > 0
            			_nValor := _VRKMRO * _nTotKm
            		EndIf
            		If _PEKMRO > 0
            			_nValor := ( _VRKMON + ( _VRKMON * _PEKMRO)/100 ) * _nTotKm
            		EndIf
            		*/
            		
            		// Forma solicitada pela Cris 29/04/2016
					_nValor := 0
            		If _VRFIXO > 0 
            			_nValor := _VRFIXO
            		Else
            		...
            		// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nValor
						EndIF
					Next
				...
				// CASO 3
				If _nTotLin == 2 .and. _nTotPeca == 1 .and. _nTotReto == 1
					/*
					_nValor := 0
            		If _VRFIXR > 0
            			_nValor := _VRFIXR
            		...
            		_nValor1 := (_nValor * _PRTEMB)/100 // Retorno
            		_nValor2 := (_nValor * (100 - _PRTEMB))/100 // Peca
            		
            		// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nValor1
						EndIF
					Next
					// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nValor2
						EndIF
					Next
					*/
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nRetorno
						EndIF
					Next
					// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nPecas
						EndIF
					Next
				...
				// CASO 4
				If _nTotLin > 1 .and. _nTotPeca == _nTotLin
            		_nValor := 0
            		If _VRFIXO > 0 
            			_nValor := _VRFIXO
            		Else
					...
					// Distribuir proporcionalmente pelo peso
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nTemp  := (_nValor * ((_nPeso * 100)/_nTotPeso))/100
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nTemp
						EndIF
					Next
            	...
            	// CASO 5
            	If _nTotLin > 1 .and. _nTotReto == _nTotLin
            		/*
            		Forma antiga
            		_nValor := 0
            		If _VRFIXR > 0
            		...
            		*/   
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							//_nValor := _nValor * ((_nPeso * _nTotPeso)/100)/100
							_nTemp := (_nValor * ((_nPeso * 100)/_nTotPeso))/100
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nTemp
						EndIF
					Next
				...
				// CASO 6
				// Frank Fuga em 04/05/16 _nTotLin estava somente >
				If _nTotLin >= 3 .and. (_nTotReto > 0 .and. _nTotPeca > 0)
					/*
					_nValor := 0
            		If _VRFIXR > 0
            			_nValor := _VRFIXR
					...
					// Total de peso para pecas
            		_nTotPP := 0
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							_nTotPP += oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
						EndIF
					Next
					
					// Total de peso para retornos
            		_nTotPR := 0
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							_nTotPR += oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
						EndIF
					Next

					// Distribuir as pecas proporcionalmente
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nValor := (_nValor2 * ((_nPeso * 100)/_nTotPP))/100
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nTemp	:= (_nValor1 * ((_nPeso * 100)/_nTotPR))/100
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nTemp
						EndIF
					Next
					*/  
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nValor := (_nPecas * ((_nPeso * 100)/_nTotPP))/100
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nTemp	:= (_nRetorno * ((_nPeso * 100)/_nTotPR))/100
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VALOR"})] := _nTemp
						EndIF
					Next
		...
		If _nVALKM+_nVALKMA > 0		
			...
			For _nX:=1 to Len(oDlgCar:aCols)	
				...
				if ZT0->ZT0_DESLOC == "S"		// Carrega Valor de Deslocamento - Cristiam Rossi em 05/02/2016
					//_nTemp4 := ZT0->ZT0_VRDES *  gdFieldGet("ZA7_KMDESL" , _nX, .F., oDlgCar:aHeader, oDlgCar:aCols) // modificado conforme linha abaixo por Frank 30/03/2016 - estava alterando o valor digitado pelo usuario
					_nTemp4 := gdFieldGet("ZA7_VALDES" , _nX, .F., oDlgCar:aHeader, oDlgCar:aCols) 
					_nTemp += _nTemp4
			    	gdFieldPut("ZA7_VALDES" , _nTemp4, _nX, oDlgCar:aHeader, oDlgCar:aCols)
				else
			    	//gdFieldPut("ZA7_VALDES" , 0      , _nX, oDlgCar:aHeader, oDlgCar:aCols) // Frank 30/03/16 segundo o Anderson devemos manter o valor informado pelo usuario, so nao vamos utilizar no calculo
				endif
				//Pedrassi Begin
...
If _lGerAcols // Tratamento especial para compatibilizar a rotina LOCT005
	...
	AAdd(aCamposSim,{"ZA6_TPTRAC","" })		// Cristiam Rossi em 05/04/2016
...
//		If ZT1_TIPTAB == cTpTraC  .and. ZT1_ITEMTB == cITTX .and. ZT1_CODCLI == cCodCli .and. ZT1_LOJCLI == cLojCli
		If ZT1_ITEMTB == cITTX .and. ZT1_CODCLI == cCodCli .and. ZT1_LOJCLI == cLojCli
			_lAchou := .T.
			Exit
		EndIF
		dbSkip()
	...
	If ZT1->ZT1_TIPREG < "4" .and. _lGerAcols // modelo de calculo utilizado na coleta LOCT005
		...
		For nPos := 1 to Len(oGetZA7:aCols)
			...
			mSQL := "SELECT ZA6_TPTRAN, ZA6_TPTRAC,ZA6_TIPFLU,ZA6_DTINI,ZA6_EMERG2,ZA6_TABCOM,ZA6_VERCOM, "
			...
			if ! EOF()
				...
				cMunic    := TZA7->ZA7_MUNICI
	     		...
	     		cTpTraC   := TZA7->ZA6_TPTRAC		// Cristiam Rossi em 05/04/2016
...
//			mSQL += " AND ZT1_TIPTAB='"+cTpTrans+"' AND ZT1_TIPFLU='"+cTpFluxo+"' "
			mSQL += " AND ZT1_TIPTAB='"+cTpTraC+"' AND ZT1_TIPFLU='"+cTpFluxo+"' "
...
//			If ZT1->ZT1_TIPTAB == cTpTraC  .and. ZT1->ZT1_ITEMTB == cITTX .and. ZT1->ZT1_CODCLI == cCodCli .and. ZT1->ZT1_LOJCLI == cLojCli
			If ZT1->ZT1_ITEMTB == cITTX .and. ZT1->ZT1_CODCLI == cCodCli .and. ZT1->ZT1_LOJCLI == cLojCli
		...
		If _aRet[2] == "S" // Tratamento especial para rateio por KM/PESO
			...
			For _nX:=1 to Len(oDlgCar:aCols)    
				If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
					...
					If _aRet[2] == "S" // todos os rateios devem ter o seqcol preenchido
						If empty(oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_SEQCOL"})])
							_lErro := .T.
						EndIF
					EndIF
		...
		// Frank 02/03/2016 
		_aRet := U_KMMOD2("1", _nX, "2")
		If _aRet[2] == "N" .and. _aRet[4] == "S" // Tratamento especial para rateio line haul
			...
			If _cTipo == "FTL"
				...
				U_AchaKM()
				U_FIATFTL()
				_nTotKm := oDlgTra:aCols[oDlgTra:nAt][Ascan(oDlgTra:aHeader,{|x|AllTrim(x[2])=="ZA6_EKMCOM"})] 

				/*                                     
				Premissa: 
				1) a tabela ZTD sera posicionada com base no ZA6_EKMCOM
				2) o total de KM vem do campo ZA6_EKMCOM
				
            	CASO 1
            	Uma unica linha com PECA                    
            		se ZTD_VRFIXO > 0
            			ZA7_VALOR := ZTD_VRFIXO
            		se nao
            			ZA7_VALOR := ZTD_VRKMON * TOTKM 
            		fim-se
            	
            	CASO 2
            	Uma unica linha com RETORNO
            		se ZTD_VRFIXR > 0
            			ZA7_VALOR := ZTD_VRFIXR
            		se ZTD_PERR > 0
            			ZA7_VALOR := ZTD_VRFIXO + (ZTD_VRFIXO * ZTD_PERR)/100
            		se ZTD_VRKMRO > 0
            			ZA7_VALOR := ZTD_VRKMRO * TOTKM
            		se ZTD_PEKMRO > 0
            			ZA7_VALOR := ( ZTD_VRKMON + ( ZTD_VRKMON * ZTD_PEKMO) ) * TOTKM
            		fim-se
            	...
            	Duas linhas ou mais somente de PECA
            		...
            		Posicionar na primeira PECA
            		ZA7_VALOR := X * ((ZA7_PESO * y)/100)/100
                ...
                */                  
				...
				_lPassou := .F.
				While !Eof() .and. ZTD_FILIAL+ZTD_TABCOM+ZTD_VERCOM+ZTD_ITTABC == xFilial("ZTD")+ZT1->ZT1_CODTAB+ZT1->ZT1_VERTAB+ZT1->ZT1_ITEMTB
					If ZTD_FAIXAD <= _nTotKm .and. ZTD_FAIXAA >= _nTotKm					
						_VRKMON		:= ZTD_VRKMON
						_VRFIXO 	:= ZTD_VRFIXO
						_VRKMRO 	:= ZTD_VRKMRO
						_PEKMRO 	:= ZTD_PEKMRO
				...
				If _nTotLin == 1 .and. _nTotPeca == 1
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nValor
						EndIF
					Next
				...
				If _nTotLin == 2 .and. _nTotPeca == 1 .and. _nTotReto == 1
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nValor1
						EndIF
					Next
					// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nValor2
						EndIF
					Next
					*/  
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "R"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nRetorno
						EndIF
					Next
					// Atualizar o valor na ZA7
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nPecas
						EndIF
					Next
				...
				If _nTotLin > 1 .and. _nTotPeca == _nTotLin
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
							If !_lPassou
								_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
								//_nTemp  := (_nValor * ((_nPeso * 100 )/_nTotPeso))/100 // alterado por frank, nao precisa realizar o rateio
								//oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nTemp
								oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nValor
								_lPassou := .T.
							EndIf
            	...
            	If _nTotLin > 1 .and. _nTotReto == _nTotLin
            		...
            		*/          
            		...
            		// Forma nova - solicitada pela Cris 29/04/2016
            		_nValor := 0
            		If _VRFIXO > 0 
            			_nValor := _VRFIXO
            		Else
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
							If !_lPassou
								_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
								//_nTemp  := (_nValor * ((_nPeso * 100)/_nTotPeso))/100 // Frank - Não precisa realizar o rateio
								//oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nTemp
								oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nValor
								_lPassou := .T.
							EndIf
				...
				If _nTotLin >= 3 .and. (_nTotReto > 0 .and. _nTotPeca > 0)
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] .and. oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_DEVEMB"})] == "P"
							_nPeso  := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_PESO"})]
							_nTemp  := (_nValor2 * ((_nPeso * 100)/_nTotPP))/100
							oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nTemp
						EndIF
					Next 
					...
					*/
            		...
            		For _nX:=1 to Len(oDlgCar:aCols)
						If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1] 
							If !_lPassou 
								oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := _nPecas + _nRetorno
								_lPassou := .T.
							Else
								oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})] := 0
							EndIF
		...
		If _nVALKM+_nVALKMA > 0		
			...
			For _nX:=1 to Len(oDlgCar:aCols)	
				...
				if ZT1->ZT1_DESLOC == "S"		// Carrega Valor de Deslocamento - Cristiam Rossi em 05/02/2016
					...
					_nTemp4 := gdFieldGet("ZA7_VRDESC" , _nX, .F., oDlgCar:aHeader, oDlgCar:aCols) 
					_nTemp += _nTemp4
	...
	If _lGerAcols
		...
		For _nX:=1 to Len(oDlgCar:aCols)
			If !oDlgCar:aCols[_nX][Len(oDlgCar:aHeader)+1]
				...
				ZA7->(dbSetOrder(3))
				ZA7->(dbSeek( oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_VIAGEM"})] ))
				
				If ZA7->(!eof())
					RecLock("ZA7",.F.)
					ZA7->ZA7_CUSTO := oDlgCar:aCols[_nX][Ascan(oDlgCar:aHeader,{|x|AllTrim(x[2])=="ZA7_CUSTO"})]
					MsUnlock()
				EndIF
...
Local _cTpTraC	:= oDlgTra:aCols[oDlgTra:nAt][Ascan(oDlgTra:aHeader,{|x|AllTrim(x[2])=="ZA6_TPTRAC"})]
...
If !_lErro
	If _cMod == "1" // Venda
		If !_lFtlv
			...
			EndIf	
		Else                        
			_nRetorno := oDlgTra:aCols[oDlgTra:nAt][Ascan(oDlgTra:aHeader,{|x|AllTrim(x[2])=="ZA6_EKMVEN"})]
		EndIF
	Else
		If !_lFtlc
			If _cCompra == "N"
			...
			EndIf
		Else
			_nRetorno := oDlgTra:aCols[oDlgTra:nAt][Ascan(oDlgTra:aHeader,{|x|AllTrim(x[2])=="ZA6_EKMCOM"})]
		EndIF
	EndIF                    
EndIF
...
/*----------------------------------------------------
Rotina que simula as Abas do LOCC001 ( ZA6 e ZA7 )
Chama os Recálculos CALCZA7 de Venda e Compra
Retorna o Valor dos Fretes de Venda e de Compra

Cristiam Rossi em 07/04/2016
----------------------------------------------------*/
User Function vCalcZA7( _Filial, _Projet, _Obra, _Viagem )
...
Local   nVlFrtV    := 0		// Valor de Retorno da Função após CALCZA7 --- Valor Frete Venda
Local   nVlFrtC    := 0		// Valor de Retorno da Função após CALCZA7 --- Valor Frete Compra

Local   aRetCols   := {}

Private oDlgTra    := nil	// ZA6
Private oDlgCar    := nil	// ZA7

Default _Filial    := cFilAnt
Default _Projet    := ""
Default _Obra      := ""
Default _Viagem    := ""

	if Empty( _Projet ) .or. Empty( _Obra ) .or. Empty( _Viagem )		// Não passou os parâmetros, retorna-se ZEROS
//		Return { 0, 0 }
		Return {}
	endif

	if cFilAnt != _Filial		// é de Outra Filial... Alteramos
		cFilAnt := _Filial
	endif

	ZA0->( dbSetOrder(1) )
	ZA0->( dbSeek( xFilial("ZA0") + _Projet ) )

	cChave     := xFILIAL(cAlias) + _Projet + _Obra
	cCondicao  := 'ZA6_FILIAL+ZA6_PROJET+ZA6_OBRA=="'+cChave+'"' 
	cFiltro    := cCondicao


// Geração Objeto oDlgTra
	AAdd(aCamposSim,{"ZA6_OBRA"  ,"V"})
	AAdd(aCamposSim,{"ZA6_SEQTRA","V"}) 
	AAdd(aCamposSim,{"ZA6_AS"    ,"V"})
	AAdd(aCamposSim,{"ZA6_CVA"   ,"V"})
...
//		if gdFieldGet("ZA7_VIAGEM", nI, .F., oDlgCar:aHeader, oDlgCar:aCols) == _Viagem
//			oDlgCar:nAt := nI
//			exit
//		endif
//	next
...
// Chama CALCZA7.PRW para atualizar os valores
	U_ACHAKM()
	
	U_FIATFTL()

	u_CalcZA7V()	// Efetua o recálculo Tabela de Vendas
	u_CalcZA7C()	// Efetua o recálculo Tabela de Compras

//	nVlFrtV := gdFieldGet("ZA7_VALOR", nI, .F., oDlgCar:aHeader, oDlgCar:aCols)		// obtemos Frete Valor Venda
//	nVlFrtC := gdFieldGet("ZA7_CUSTO", nI, .F., oDlgCar:aHeader, oDlgCar:aCols)		// obtemos Frete Valor Compra

	DTQ->( dbSetOrder(1) )

	for nI := 1 to len( aCols )

		_cViage := gdFieldGet("ZA7_VIAGEM", nI, .F., oDlgCar:aHeader, oDlgCar:aCols)	// obtemos Frete Valor Venda
		
		if DTQ->( dbSeek(xFilial("DTQ") + _cViage ) ) .and. Empty( DTQ->DTQ_SERCTR )

			nVlFrtV := gdFieldGet("ZA7_VALOR", nI, .F., oDlgCar:aHeader, oDlgCar:aCols)		// obtemos Frete Valor Venda
			nVlFrtC := gdFieldGet("ZA7_CUSTO", nI, .F., oDlgCar:aHeader, oDlgCar:aCols)		// obtemos Frete Valor Compra

			aadd( aRetCols, { _cViage, nVlFrtV, nVlFrtC } )
		endif
	next
	...
	restArea( aArea )
//Return { nVlFrtV, nVlFrtC }

Return aClone( aRetCols )
